...
public:
    WaypointFollower() : Node("waypoint_follower") {
        ...
        slow_pid_x_ = std::make_unique<PIDController>(1.0, 0.1, 0.2, 0.1);  // X축 PID
        slow_pid_y_ = std::make_unique<PIDController>(1.0, 0.1, 0.2, 0.1);  // Y축 PID
        slow_pid_z_ = std::make_unique<PIDController>(2.0, 0.5, 0.2, 0.1);  // Z축 PID (진동 감소를 위한 조정)
        ...
        // slow_pid_z_ = std::make_unique<PIDController>(2.0, 0.5, 0.2, 0.1);   // Z축 PID
        ...
        // 벽 접근 속도 제한 추가
        wall_approach_max_velocity_ = 0.6f;  // 벽 접근 최대 속도 설정 (0.6 m/s) 최적의 안정값이 0.6입니다.
        //wall_approach_max_acceleration_ = 0.3f;  // 벽 접근 최대 가속도 설정 (0.3 m/s^2) *(아직)가속도 제한은 사용하지 않음(향후 생각해보기)
