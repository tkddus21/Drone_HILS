...
public:
    WaypointFollower() : Node("waypoint_follower"),
        ...
        pid_x_(std::make_unique<PIDController>(1.0, 0.1, 0.2, 0.1)),
        pid_y_(std::make_unique<PIDController>(1.0, 0.1, 0.2, 0.1)),
        pid_z_(std::make_unique<PIDController>(2.0, 0.5, 0.2, 0.1)),
        wall_approach_max_velocity_(0.2f),  // 이미 멤버 변수로 선언되어 있음
...
private:
    ...
    void publishOffboardControlMode() {
        ...
        msg.velocity = true;  // 속도 제어도 활성화 (하이브리드 모드)
    ...
    void publishTrajectorySetpoint() {
        ...
        if (is_moving_to_wall_) {
            ...
            // 단위 방향 벡터 계산
            if (distance > 0.001) {
                wall_direction_unit_[0] = static_cast<float>(dx / distance);
                wall_direction_unit_[1] = static_cast<float>(dy / distance);
                wall_direction_unit_[2] = static_cast<float>(dz / distance);
            }
            ...
            double z_correction = pid_z_->compute(waypoints_[current_waypoint_index_].z, current_position_z_);
    ...
    void timer_callback() {
        ...
        if (is_waiting_at_waypoint_) {
            ...
            if (waiting_time >= waypoint_wait_time_) {
                ...
                double heading_rad = current_yaw_;
                
                // 로그로 현재 방향 확인
                RCLCPP_INFO(this->get_logger(), 
                    "현재 드론 헤딩: %.2f도 (%.2f라디안)",
                    heading_rad * 180.0 / M_PI, heading_rad);
                
                // 정면 방향으로 이동하도록 벡터 계산
                double delta_x = move_distance * std::cos(heading_rad);
                double delta_y = move_distance * std::sin(heading_rad);
                
                // 최종 목표 위치 설정
                wall_target_x_ = current_position_x_ + delta_x;
                wall_target_y_ = current_position_y_ + delta_y;
                wall_target_final_x_ = wall_target_x_;
                wall_target_final_y_ = wall_target_y_;
                
                // 벽 접근 상태 설정
                is_moving_to_wall_ = true;
                use_slow_pid_ = true;  // 저속 PID 사용
                wall_approach_max_velocity_ = 0.5f;  // 속도 제한 0.5m/s (더 안정적인 접근을 위해)
                
                // 이동 방향 벡터 계산 (target_update_node.cpp 방식 참고)
                double distance = std::sqrt(delta_x*delta_x + delta_y*delta_y);
                if (distance > 0.001) {
                    // 단위 방향 벡터 계산
                    wall_direction_unit_[0] = static_cast<float>(delta_x / distance);
                    wall_direction_unit_[1] = static_cast<float>(delta_y / distance);
                    wall_direction_unit_[2] = 0.0f;
                } else {
                    // 거리가 너무 가까우면 기본값 설정
                    wall_direction_unit_[0] = std::cos(heading_rad);
                    wall_direction_unit_[1] = std::sin(heading_rad);
                    wall_direction_unit_[2] = 0.0f;
                }
                ...
                RCLCPP_INFO(this->get_logger(), 
                    "벽 접근 시작: 현재 위치(%.2f, %.2f) -> 목표 위치(%.2f, %.2f), 이동 거리: %.2fm, 속도: %.1fm/s",
                    current_position_x_, current_position_y_, wall_target_x_, wall_target_y_, 
                    move_distance, wall_approach_max_velocity_);
                
                // 방향 벡터 로깅
                RCLCPP_INFO(this->get_logger(), 
                    "이동 방향 벡터: (%.2f, %.2f, %.2f)",
                    wall_direction_unit_[0], wall_direction_unit_[1], wall_direction_unit_[2]);
            } else {
                RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
        ...
        if (is_moving_to_wall_) {
            ...
            // 경과 시간 계산 (target_update_node.cpp 방식 참고)
            double elapsed_seconds = (this->get_clock()->now() - wall_approach_start_time_).seconds();
            
            // 현재까지 이동해야 할 거리 계산 (최대 속도로 제한)
            float current_distance = std::min(static_cast<float>(elapsed_seconds * wall_approach_max_velocity_), 
            ...
            // 진행률 계산 및 로깅
            float progress = (distance > 0.001f) ? ((1.0f - distance / wall_distance) * 100.0f) : 100.0f;
            
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                "벽 접근 중: 현재(%.2f, %.2f) -> 목표(%.2f, %.2f), 남은 거리: %.2fm, 속도: %.1fm/s, 진행률: %.1f%%",
                current_position_x_, current_position_y_, wall_target_x_, wall_target_y_, 
                distance, wall_approach_max_velocity_, progress);
            
            // 목표 지점에 도달했는지 확인 (10cm 이내)
            if (distance < 0.1) {
                // 벽 접근 완료, 대기 상태로 전환
                RCLCPP_INFO(this->get_logger(), 
                    "벽 접근 완료! 현재 위치: (%.2f, %.2f), 목표 위치: (%.2f, %.2f)",
                    current_position_x_, current_position_y_, wall_target_x_, wall_target_y_);
                ...
                if (distance_diff < 0.1) {  // 10cm 이내 차이면 정렬되었다고 판단
                    ...
                    is_aligned_with_wall_ = false;
                    RCLCPP_INFO(this->get_logger(), 
                        "헤드 방향이 벽면과 정렬되지 않았습니다. 왼쪽=%.2fm, 오른쪽=%.2fm, 차이=%.2fm",
                        left_distance_, right_distance_, distance_diff);
                }
            }
        }
    ...
    // 클래스 멤버 변수로 추가
    std::array<float, 3> wall_direction_unit_ = {0.0f, 0.0f, 0.0f};
    float wall_approach_max_velocity_;

    // 벽 접근 시작 함수 개선
    void startWallApproach() {
        // 벽과의 거리 계산 (라이다 데이터 사용)
        float wall_distance = (left_distance_ + right_distance_) / 2.0f;
        
        // 벽과의 거리가 1.5m가 되는 지점까지 이동해야 할 거리
        float target_distance = 1.5f;  // 목표 거리: 1.5m
        float move_distance = wall_distance - target_distance;
